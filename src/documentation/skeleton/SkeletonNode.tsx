...
import {
  SkullDocsBaseClass,
  SkullDocsEntry,
  SkullDocsFunctionParameter,
} from "../../language-server/skulldocs";
...
interface SkullDocEntryNodeProps extends BoxProps {
  docs: SkullDocsEntry;
  anchor?: Anchor;
  parentType?: string;
}
  ...
  return (
    <Highlight anchor={anchor} active={active} id={id} disclosure={disclosure}>
      ...
      >
        ...
        />
        <SkeletonNodeChildren docs={docs} anchor={anchor} />
      </Stack>
    </Highlight>
  );
};

interface SkeletonNodeSelfProps {
  docs: SkullDocsEntry;
  showMore: boolean;
 ...
 */
const SkeletonNodeSelf = ({
  docs,
  showMore,
  onToggleShowMore,
}: SkeletonNodeSelfProps) => {
...
interface SkeletonNodeChildrenProps {
  anchor: Anchor | undefined;
  docs: SkullDocsEntry;
}

const SkeletonNodeChildren = ({
  docs,
  anchor,
}: SkeletonNodeChildrenProps) => {
...
const buildSignature = (
  kind: string,
  params: SkullDocsFunctionParameter[] | undefined,
  detailed: boolean
...
const isInitOrOtherNonDunderMethod = (c: SkullDocsEntry) =>
  !c.name.endsWith("__") || c.name === "__init__";

const filterChildren = (
  children: SkullDocsEntry[] | undefined
...
const BaseClasses = ({ value }: { value: SkullDocsBaseClass[] }) => {
  return (
    <Text pl={2}>
      <FormattedMessage
